...
function gameLoop() {
    ...
    if (delta > 5 && game.permanentUpgrades.offlineProduction.level > 0 && offlineEnabled) {
        ...
        game.starShards += game.starShardsPerSecond * delta * offlineMultiplier;
        game.starShardsTotal += game.starShardsPerSecond * delta * offlineMultiplier;
        ...
        game.starShards += game.starShardsPerSecond * delta;
        game.starShardsTotal += game.starShardsPerSecond * delta;
...
document.body.addEventListener("click", function(e) {
    if (e.target.tagName === "BUTTON" || e.target.tagName === "SPAN" || e.target.tagName === "DIV") {
        if ([...e.target.classList].some(c => c.includes('zoom')) || 
            ...
            [...e.target.classList].some(c => c.includes('bulk-buy-toggle')) || [...e.target.classList].some(c => c.includes('utility-btn')) || [...e.target.classList].some(c => c.includes('forge-button')) || [...e.target.classList].some(c => c.includes('forge-section'))) {
            return;
        }
