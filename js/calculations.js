...
export function calculateProduction() {
    ...
    for (let key in game.upgrades) {
        const upgrade = game.upgrades[key];
        if (upgrade.purchased && upgrade.resourceType === 'starShards') {
    ...
    for (let key in game.buildings) {
        ...
        if (building.produces === 'all') {
            ...
            starShardsTotal += production * 0.01;
        ...
        if (building.produces === 'stardust') {
            stardustTotal += production;
        } else if (building.produces === 'starShards') {
            starShardsTotal += production;
        } else if (building.produces === 'nebulaCores') {
    ...
    for (let key in game.upgrades) {
        ...
        if (upgrade.purchased && upgrade.crossBonus) {
            ...
            if (bonus.from === 'starShards' && bonus.to === 'stardust') {
                stardustTotal += starShardsTotal * bonus.rate;
            } else if (bonus.from === 'nebulaCores' && bonus.to === 'starShards') {
                starShardsTotal += nebulaCoresTotal * bonus.rate;
    ...
    game.starShardsPerSecond = starShardsTotal;
